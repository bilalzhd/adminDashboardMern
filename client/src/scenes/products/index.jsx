...
const Products = () => {
    ...
    return (
        <Box m="1.5rem 2.5rem">
            <Header title='PRODUCTS' subtitle='List of the products' />
            {data && !isLoading ? (
                <Box mt="20px" display="grid" gridTemplateColumns="repeat(4, minmax(0, 1fr))" justifyContent="space-between" rowGap="20px" columnGap="1.33%" sx={{
                    "& > div": {gridColumn: isDesktop ? undefined : "span 4"}
                        ...
                        {data.map(({_id, name, description, price, rating, supply, category, stat}) => (
                            <Product key={_id} _id={_id} name={name} description={description} price={price} rating={rating} supply={supply} category={category} stat={stat}/>
                        ))}
                </Box>
            ): (<>Loading...</>)}
        </Box>
